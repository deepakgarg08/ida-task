 ...
 * 
 * @RestController public class UserController_BeforeHashing { private
 * RDFConnectionRemoteBuilder builder =
 * RDFConnectionFuseki.create().destination("http://localhost:3030/user");
 * 
 ...
 * "PREFIX ab:<http://userdata/#>\r\n" + "INSERT DATA{ab:" + userName +
 * " dc:name \"" + name + "\"; dc:username \"" + userName +
 * "\" ; dc:password \"" + password + "\" .}"); conn.update(request);
 ...
 * "PREFIX ab:<http://userdata/#>\r\n" + "INSERT DATA{ab:" + name +
 * " dc:name \"" + name + "\"; dc:username \"" + userName +
 * "\" ; dc:password \"" + password + "\" .}"); conn.update(request); } return
 * true; }
 * 
 * @GetMapping("/selecta") public String select() { // In this variation, a
 * connection is built each time. try (RDFConnectionFuseki conn =
 * (RDFConnectionFuseki) builder.build()) { QueryExecution qExec =
